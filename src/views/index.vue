    ...
    <el-row :gutter="2">
        ...
        <el-card style="margin: 10px">
          <template #header>
            <div class="card-header">
              <span style="font-size: 22px; font-weight: bold; color: red">LingDu后台管理框架</span>
            </div>
          </template>
      ...
      <el-col :sm="24" :lg="12">
        <el-card style="margin: 10px; min-height: calc(100vh - 110px)">
          <template #header>
            <div class="card-header">
              <span style="font-size: 22px; font-weight: bold; color: red">更新日志</span>
          ...
          <el-timeline :reverse="reverse">
            ...
            >
              <el-card style="margin-right: 10px">
                <template #header>
                  <div class="card-header">
                    <span><span class="spanFont">标题: </span>{{ activity.title }}
                      <span class="spanFont">web版本: </span>{{ activity.app_version }}
                      <span class="spanFont">后端版本: </span>{{ activity.backend_version }}
                      <span class="spanFont">时间: </span>{{ activity.updated_at }}</span>
                    <el-button
                      v-hasPermi="['system/update_log/add']"
                      class="button"
                      link
  ...
  proxy.$refs["formRef"].validate((valid) => {
    if (valid) {
      ...
      if (form.value.id !== undefined) {
        updateUpdateInfo(form.value).then(async (_response) => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          await get_all_update_info();
        });
      } else {
        addUpdateInfo(form.value).then(async (_response) => {
