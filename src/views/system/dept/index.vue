<template>
  <div class="app-container">
    ...
    <el-table
      v-if="refreshTable" v-loading="loading" :data="deptList"
      row-key="dept_id"
      :default-expand-all="isExpandAll" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }"
    >
      <el-table-column prop="dept_name" label="部门名称" width="260" />
      <el-table-column prop="order_num" label="排序" width="200" />
      ...
      <el-table-column
        v-hasPermi="['system/dept/edit','system/dept/delete','system/dept/add']" label="操作" align="center"
        class-name="small-padding fixed-width"
      >
        <template #default="scope">
          <el-button v-hasPermi="['system/dept/edit']" link icon="Edit" @click="handleUpdate(scope.row)">
            修改
          </el-button>
          <el-button v-hasPermi="['system/dept/add']" link icon="Plus" @click="handleAdd(scope.row)">
            新增
          </el-button>
          <el-button
            v-if="scope.row.parent_id != '0'" v-hasPermi="['system/dept/delete']" link
            icon="Delete"
        ...
        <el-row>
          <el-col v-if="form.parent_id !== '0'" :span="24">
            <el-form-item label="上级部门" prop="parent_id">
              <tree-select
                v-model:value="form.parent_id" :options="deptOptions" :obj-map="{
                  value: 'dept_id',
                  label: 'dept_name',
                  children: 'children',
                }"
...
function getList() {
  ...
  listDept(queryParams.value).then((response) => {
    deptList.value = proxy.handleTree(response.list, "dept_id", "parent_id");
    loading.value = false;
  });
}
/** 取消按钮 */
...
async function handleUpdate(row) {
  ...
  const response = await getDept({ dept_id });
  form.value = response;
  open.value = true;
  title.value = "修改部门";
}
...
function handleDelete(row) {
  proxy.$modal
    .confirm("是否确认删除名称为\"" + row.dept_name + "\"的数据项?")
    .then(function () {
      return delDept({ dept_id: row.dept_id });
    })
    .then(() => {
      getList();
